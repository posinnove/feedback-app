...
export async function sendVerificationEmail(
    ...
    const path = entityType === 'user' ? 'users' : 'companies';
    const verifyUrl = `${FRONTEND_URL}/auth/${path}/verify-email?token=${token}`;
...
export async function sendPasswordResetEmail(
    ...
    const path = entityType === 'user' ? 'users' : 'companies';
    const resetUrl = `${FRONTEND_URL}/auth/${path}/reset-password?token=${token}`;
